...
export const optionalVarsSchema: Schema<IOptionalVarsSchema> = {
  ...
  sgid: {
    ...
    privateKey: {
      doc:
        'The path to the private key to decrypt payloads from sgID. Also used for JWT signing',
      format: '*',
      default: '',
    ...
    publicKey: {
      doc:
        'The path to the public key given to sgID. Also used to verify JWTs created by FormSG',
      format: '*',
      default: '',
      env: 'SGID_PUBLIC_KEY',
    },
    redirectUri: {
      doc:
        'The callback uri that sgID will pass the authorization code and state to',
      format: '*',
      default: '',
